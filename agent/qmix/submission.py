...
class RLAgent(object):
    def __init__(self, obs_dim, act_dim, num_agent):
        ...
        self.device = device
        self.actor = Actor(obs_dim, act_dim, num_agent).to(self.device)
...
def logits2action(logits):
    # 根据输出的分布(logits)随机采样到动作(actions)
...
actor_net = os.path.dirname(os.path.abspath(__file__)) + "/{0}.pth".format(WEIGHT_PATH)
